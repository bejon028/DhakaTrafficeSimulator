...
public class DrawingStep extends JPanel implements ActionListener, MouseListener, MouseMotionListener
{
	...
	this.addMouseMotionListener(this);
        ...
        VehicleLoad vl = new VehicleLoad(linkList);
	...
	ourGraphics.setTransform(at);
        ...
        int[][] lastEmptyStrip = new int[demandList.size()][];
        ...
        for(index=0; index<demandList.size(); index++)
        {
            ...
            if(nextGenerationTime.get(index)==simulationStep)
            {
                ...
                lastEmptyStrip[index]=new int[numPath];
                /*for(int i=0; i<numPath; i++)
                {
                    Path path = demandList.get(index).getPath(i);
                    if(!path.isPositiveDir())lastEmptyStrip[index][i] = linkList.get(path.getLinkIndex(0)).getSegment(0).getStripCount()/2;
                }*/
                ...
                for(index2=0;index2<numberOfVehiclesToGenerate.get(index);index2++)
                {
                    ...
                    */
                    int limit = firstLink.getSegment(0).getStripCount();
                 if(path.isPositiveDir())
                 {
                    
                    
                    for(index3=lastEmptyStrip[index][pathSelected];index3+numStripRequired-1<(limit/2);index3++) //last change /2
                    {
                        int index4,flag=1;
                        ...
                        if(flag==1)   //if enough space found empty
                        {
                            ...
                            lastEmptyStrip[index][pathSelected]=index3+numStripRequired;
                            break;
                        }
                ...
                else
                {
                        ...
                        if(lastEmptyStrip[index][pathSelected]==0)lastEmptyStrip[index][pathSelected] = limit-1;
                        limit = limit/2;
                    
                    for(index3=lastEmptyStrip[index][pathSelected];index3-numStripRequired+1>=limit;index3--) //last change /2
                    {
                        int index4,flag=1;
                        //loops to check whether required strips in segment are empty
                        for(index4=index3;index4>index3-numStripRequired;index4--)
                        ...
                        if(flag==1)   //if enough space found empty
                        {
                            //my change for lane change-> firstLink
                            Vehicle v = new Vehicle(speedSelected,accSelected,typeSelected,firstLink,firstLink.getSegment(0),
                                firstLink.getSegment(0).getStrip(index3-numStripRequired+1),index,pathSelected,0,path.isPositiveDir());     
                            vehicleList.add(v);
    ...
    public void changeLane()
    {
        ...
        for(index=0;index<vehicleList.size();index++)
        {
            ...
            Path pth = demandList.get(cv.getDemandIndex()).getPath(cv.getPathIndex());
            int currentlinkindex=pth.getLinkIndex(cv.getPathLinkIndex());
            Link currentLink=linkList.get(currentlinkindex);
            int lastLinkInPathIndex = pth.pathLength()-1;
            int nextLinkIndex;
            if(cv.hasGoneDistance() && cv.getPathLinkIndex()!=lastLinkInPathIndex)
            {
                nextLinkIndex=pth.getLinkIndex(cv.getPathLinkIndex()+1);
                //Link nextLinkOfPath=linkList.get(nextLinkIndex);

                String direction;
                int dir;
                if(pth.isPositiveDir())dir=currentLink.directionOfNextLink(nextLinkIndex);
                else dir = linkList.get(nextLinkIndex).directionOfNextLink(currentlinkindex);
                if(dir==0)direction="straight";
                else if(dir==1){
                    //if(pth.isPositiveDir())
                    //{
                        direction="left";
                        cv.moveleft();
                    //}
...
//            }
    ...
    public void drawVehicle(Graphics g,Vehicle v)
    {
        ...
        double xa = seg.getStartingPntX()*mpRatio;
        double ya = seg.getStartingPntY()*mpRatio;
        double xb = seg.getEndPntX()*mpRatio;
        double yb = seg.getEndPntY()*mpRatio;
        //Using internally section or ratio formula,it finds the coordinates along which vehicles are
        double xp,yp,xq,yq;
        int mult = 1;
        if(v.getDir())
        {
            xp = (v.getDistance()*xb+(segmentLength-v.getDistance())*xa)/segmentLength;
            yp = (v.getDistance()*yb+(segmentLength-v.getDistance())*ya)/segmentLength;
            xq = ((v.getDistance()+length)*xb+(segmentLength-(v.getDistance()+length))*xa)/segmentLength;
            yq = ((v.getDistance()+length)*yb+(segmentLength-(v.getDistance()+length))*ya)/segmentLength;
        }
        else
        {
            /*
            double width = 2*seg.getStripCount()*stripPixelCount;
            double startX = Math.round(returnX4(xa,ya,xb,yb,width));
            double startY = Math.round(returnY4(xa,ya,xb,yb,width));
            double endX = Math.round(returnX3(xa,ya,xb,yb,width));
            double endY = Math.round(returnY3(xa,ya,xb,yb,width));
            xp =(v.getDistance()*endX+(segmentLength-v.getDistance())*startX)/segmentLength;
            yp =(v.getDistance()*endY+(segmentLength-v.getDistance())*startY)/segmentLength;  //seg.getStripCount()*stripPixelCount+ 
            xq =((v.getDistance()+length)*endX+(segmentLength-(v.getDistance()+length))*startX)/segmentLength;
            yq =((v.getDistance()+length)*endY+(segmentLength-(v.getDistance()+length))*startY)/segmentLength;  //seg.getStripCount()*stripPixelCount+  
            */
            mult = -1;
            xp =(v.getDistance()*xa+(segmentLength-v.getDistance())*xb)/segmentLength;
            yp =(v.getDistance()*ya+(segmentLength-v.getDistance())*yb)/segmentLength;  //seg.getStripCount()*stripPixelCount+ 
            xq =((v.getDistance()+length)*xa+(segmentLength-(v.getDistance()+length))*xb)/segmentLength;
            yq =((v.getDistance()+length)*ya+(segmentLength-(v.getDistance()+length))*yb)/segmentLength;  //seg.getStripCount()*stripPixelCount+
            
        }
        
    	//finds the coordinates of vehicles starting and ending upper points depending on which strip their upper(left) portion are.
    	int x1 = (int) Math.round(returnX3(xp,yp,xq,yq,(mult*v.getStrip().getStripIndex())*stripPixelCount));
	int y1 = (int) Math.round(returnY3(xp,yp,xq,yq,(mult*v.getStrip().getStripIndex())*stripPixelCount));
	int x2 = (int) Math.round(returnX4(xp,yp,xq,yq,(mult*v.getStrip().getStripIndex())*stripPixelCount));
	int y2 = (int) Math.round(returnY4(xp,yp,xq,yq,(mult*v.getStrip().getStripIndex())*stripPixelCount));
		
	int width = (int) Math.ceil(v.getWidth())*stripPixelCount;
	
        //finds the coordinates of perpendicularly oppsite lower(right) points
	int x3 = (int) Math.round(returnX3(xp,yp,xq,yq,(mult*v.getStrip().getStripIndex())*stripPixelCount+mult*width));
	int y3 = (int) Math.round(returnY3(xp,yp,xq,yq,(mult*v.getStrip().getStripIndex())*stripPixelCount+mult*width));
	int x4 = (int) Math.round(returnX4(xp,yp,xq,yq,(mult*v.getStrip().getStripIndex())*stripPixelCount+mult*width));
	int y4 = (int) Math.round(returnY4(xp,yp,xq,yq,(mult*v.getStrip().getStripIndex())*stripPixelCount+mult*width));
