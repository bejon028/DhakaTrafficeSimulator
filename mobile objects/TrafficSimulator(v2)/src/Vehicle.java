...
public class Vehicle {
	...
	//private double stripWidth = DrawingStep.isStripBased ? 0.5 : 2.5;  //last changed 0.5 on 15/02/16
                ...
                int dmandIndex, int pthIndex, int pthLinkIndex)
	{
		...
		numStrip = numStripOccupied(width);
                numFpStrip = (int) Math.ceil(width/Strip.fpStripWidth);
	...
	public void moveright()
	{
		...
		if(x+numStrip<segment.getStripCount() 
	...
	{
		int x = strip.getStripIndex();
		if(x!=segment.getFpStripCount() && segment.getStrip(x-1).isGapForStripChange(this)==true
                        && !this.reachedJunction
                        && isMovedForward)
		{
                        setStrip(segment.getStrip(x-1));
	...
	*forward and sets the passedSensor variable true when it passes by the distance.
	...
	{
		double widthCon = width/Strip.stripWidth;
		int numStrips = (int) Math.ceil(widthCon);
		return numStrips;
	}
    ...
    {
        String vehicle = this.demandIndex+" "+this.pathIndex+" "+this.pathLinkIndex+" "+this.segment.getSegmentIndex()+" "+this.strip.getStripIndex()+" "+this.distance;
        return vehicle;
    }
}
